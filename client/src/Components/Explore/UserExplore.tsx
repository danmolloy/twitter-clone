...
export const UserExplore = (props: {user: ExploreUser, currentUserHandle: string | undefined}) => {
  return (
       <div className="user-explore border-b flex flex-row justify-between hover:bg-gray-100 ">
        <Link id="explore-user-link" to={`/${props.user.handle.slice(1)}`} className="flex flex-row m-4">
        <img src={props.user.profilePic} alt={`${props.user.name}`} className="w-12 sm:w-16 h-auto rounded-full " />
