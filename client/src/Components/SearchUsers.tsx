...
export const SearchUsers = (props: any) => {
  ...
  return (
    <div className=" w-3/4 sm:w-1/2 h-1/2 border shadow mt-4 z-10 fixed bg-white">
      ...
      {data && data.currentUser.follows.map((i: any) => 
        <button key={i.handle} onClick={() => handleClick(i.handle)} className=" w-full border hover:bg-gray-100 flex flex-row item-start">
          <p className="px-2">{i.name}</p>
        </button>
      )}
      {data && JSON.stringify(data.currentUser.follows.map((i: any) => i.handle))}
    </div>
  )
}